...
notification path, and subscribing to it is what kills the connection on a
local BlueZ adapter (confirmed on hardware, see ARCHITECTURE.md). So this
coordinator's periodic cycle is a connection-health check, not a state
fetch: it makes sure an authenticated session exists, and entities report the
last state they themselves commanded rather than anything read back from the
device (`assumed_state` in HA terms).
...
from cync_lan.ble_mesh import BleMeshError, BleMeshSession
...
class CyncBleCoordinator(DataUpdateCoordinator[None]):
    ...
    async def _connect_to(self, mac: str) -> BleMeshSession | None:
        ...
        except Exception as exc:  # noqa: BLE001 - any one candidate failing is routine
        ...
        try:
            ...
            _LOGGER.warning("%s: pairing handshake failed via %s: %s", self.mesh_name, mac, exc)
    ...
    async def _with_retry(self, call: Any) -> None:
        ...
        try:
            await call(session)
        except Exception as exc:  # noqa: BLE001 - covers both BleMeshError and a
        ...
        # DEFAULT_REFRESH_INTERVAL_SECONDS later.
    ...
    async def async_shutdown_session(self) -> None:
        if self._client is not None:
            ...
            except Exception:  # noqa: BLE001 - best-effort teardown on unload
                _LOGGER.debug("%s: error disconnecting on unload", self.mesh_name, exc_info=True)
        self._client = None
        self.session = None
